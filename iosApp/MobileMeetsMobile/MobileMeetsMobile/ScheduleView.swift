...
class ScheduleViewModelWrapper: ObservableObject {
    ...
    init() {
        self.viewModel = KoinInit.shared.getScheduleViewModel()
...
struct ScheduleView: View {
                            ...
                            VStack(alignment: .leading, spacing: 8) {
                                ...
                                HStack(spacing: 12) {
                                    Circle()
                                        .fill(
                                            LinearGradient(
                                                colors: [Color.googleBlue, Color.googleGreen],
    ...
    func trackColor(_ track: Track) -> Color {
        switch track {
        case .aiMl: return Color.googleBlue
        case .android: return Color.googleGreen
        case .web: return Color.googleYellow
        case .cloud: return Color.googleRed
        case .firebase: return Color(hex: 0xFF6D00)
        case .flutter: return Color(hex: 0x42A5F5)
        case .design: return Color(hex: 0xA142F4)
        default: return Color.googleBlue
        }
    }
}
...
struct DayTabView: View {
    ...
    var body: some View {
        Button(action: action) {
            ...
            .background(
                isSelected
                    ? AnyShapeStyle(LinearGradient(colors: [Color.googleBlue, Color.googleGreen], startPoint: .topLeading, endPoint: .bottomTrailing))
                ...
                HStack {
                    ...
                    Button(action: onBookmark) {
                        Image(systemName: session.isBookmarked ? "star.fill" : "star")
                            .foregroundColor(session.isBookmarked ? Color.googleYellow : .white.opacity(0.25))
                    }
                }
